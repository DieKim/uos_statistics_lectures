...
print(k) # reject이 많이 되면 별로 효율적이지 않음
...
b <- burn + 1 # 앞에 1000개를 버리고 1001개부터
x <- X[b:N, ]
...
plot(x, main="", cex=.5, xlab=bquote(X[1]), # 번인 제외 plot; 반비례
...
N <- 2000
sigma <- c(.05,.5,2,16) # sigma 값에 따라서 기각이 달라짐
...
cbind(MCE(30,b.rw1),MCE(30,b.rw2),MCE(30,b.rw3),MCE(30,b.rw4)) # 작을수록 좋음

# 그래프를 이용하는 방법
acf.plot <- function(x){ # 시계열의 ACF
  index <- c(1:N)
  acf(x, type="correlation", plot=TRUE, main="Autocorrelation")
}

par(mfrow=c(2,2))
acf.plot(b.rw1); acf.plot(b.rw2); acf.plot(b.rw3); acf.plot(b.rw4) # 자기상관이 빨리 죽는게 좋음ㅁ
